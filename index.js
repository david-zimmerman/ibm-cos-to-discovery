...
async function main(args) {
  ...
  if (!params.bucket || !params.key || !cos) {
    result.message =
      "bucket name, key, and apikey are required for this operation.";
    throw result;
  }
...
function getParamsCOS(args, COS) {
  ...
  const endpoint =
    args.endpoint || "s3.us.cloud-object-storage.appdomain.cloud";
  const ibmAuthEndpoint =
    args.ibmAuthEndpoint || "https://iam.cloud.ibm.com/identity/token";
  const apiKeyId =
    args.apikey ||
    args.apiKeyId ||
    bxCredsApiKey ||
    process.env.__OW_IAM_NAMESPACE_API_KEY;
...
csvStr = result.body.fromString(csvStr).then(csvRow => {
